...
public class Board {
	...
	public int hamming() { // number of blocks out of place
		int count = 0;
		for (int i = 0; i < N * N; i++) // compare board[1] through board[N^2-1]
										// with goal
			if (board[i] != i + 1 && board[i] != 0) // count for blocks in wrong
													// place
				count++;
		return count;
	}
	...
	private int manhattan(int goal, int current) { // return manhattan distance
													// of a misplaced block
	...
	public Board twin() { // a board obtained by exchanging two adjacent blocks
		...
		Board twin;
		if (N == 1)
			return null; // check if twin board exists
		twin = new Board(board);

		if (board[0] != 0 && board[1] != 0)
			exch(twin, 0, 1); // if the first two blocks in first row is not
								// empty, exchange them.
		else
			exch(twin, N, N + 1); // otherwise, exchange the first two blocks on
									// second row.
		return twin;
	}

	private void exch(Board a, int i, int j) { // exchange two elements in the
												// array
		int temp = a.board[i];
		a.board[i] = a.board[j];
		a.board[j] = temp;
